...
Store.prototype.getDailyFiles = function(count, cb) {
    var query = {};
    if (count) query.limit = count;
    db.view('node3p-web', 'getDailyFiles', query, function(err, results) {
        if (err) throw new Error(JSON.stringify(err));
        if (cb) cb(err, results.rows);
    });
};
...
Store.prototype.getAlbums = function(cb) {
    db.view('node3p-web', 'getAlbums', {group:true,limit:10}, function(err, results) {
        if (err) throw new Error(JSON.stringify(err));
        if (cb) cb(err, results.rows);
    });
};